...
def profile(fn, mask_nnz, mode="fwd", warmup=25, rep=100):
    ...
    flops_per_matmul = 2.0 * N_HEADS * mask_nnz * D_HEAD
...
def test_flash_attention(dtype=torch.float16, device="cuda"):
    q = torch.randn(
        (BATCH, N_HEADS, M_CTX, D_HEAD), dtype=dtype, device=device, requires_grad=True
    )
    k = torch.randn(
        (BATCH, N_HEADS, N_CTX, D_HEAD), dtype=dtype, device=device, requires_grad=True
    )
    v = torch.randn(
        (BATCH, N_HEADS, N_CTX, D_HEAD), dtype=dtype, device=device, requires_grad=True
    )
