...
class FIFOContextPool(object):
    ...
    def __init__(self, runner: BuiltinRunner, sampling_config: SamplingConfig, size: int):
...
def profile_bing_chat(
    ...
    if shared:
        shared_fill = Fill(pid=0, tid=0, context_id=batch_size, parent_context_id=-1, token_ids=prompt_token_ids[0][:shared_ids])
        e2e_time_sf, model_time_sf = runner.run_iter([shared_fill])
    else:
        e2e_time_sf, model_time_sf = 0, 0

    for prompt_idx in range(batch_size):
        context_pool.push(prompt_token_ids[prompt_idx][shared_ids:], parent_context_id, gen_limits[prompt_idx])
    num_voids = 0
    while num_voids < batch_size:
        num_voids = context_pool.run()
        print(f'[#{prompt_idx + num_voids + 1 - batch_size:0>2} - #{prompt_idx:0>2}] / {len(prompt_token_ids)}')
    ...
    print(f'  Shared Fill      : {e2e_time_sf / 1e9:7.3f} s, {model_time_sf / 1e9:7.3f} s')
    print(f'Diverged Fill + Gen: {e2e_time_dfg / 1e9:7.3f} s, {model_time_dfg / 1e9:7.3f} s')
    print(f'              Total: {total_time / 1e9:7.3f} s')
    return e2e_time_sf, model_time_sf, e2e_time_dfg, model_time_dfg, total_time
...
if __name__ == '__main__':
    ...
    if use_sample:
        log_path = './shared_prompt_exp_2.csv'
        params = [args.mode, batch_size]
    else:
        log_path = './shared_prompt_exp_1.csv'
        params = [args.mode, max_gen_length]
    params = ', '.join([str(p) for p in params])

    try:
        results = profile_bing_chat(shared, use_sample, max_gen_length, batch_size, attn_func)
